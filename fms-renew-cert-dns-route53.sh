...
if [[ $TEST_CERTIFICATE -eq 1 ]] ; then
    ...
    if [[ $FORCE_RENEW -eq 1 ]] ; then
        sudo -E certbot renew $DNS_PLUGIN --cert-name $DOMAIN --force-renew --config-dir "$CERTBOTPATH" --work-dir "$CERTBOTPATH" --logs-dir "$CERTBOTPATH"
    else
        sudo -E certbot renew $DNS_PLUGIN --cert-name $DOMAIN --config-dir "$CERTBOTPATH" --work-dir "$CERTBOTPATH" --logs-dir "$CERTBOTPATH"
    fi
fi
